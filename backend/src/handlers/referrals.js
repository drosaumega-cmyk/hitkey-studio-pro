...
export class ReferralHandler {
    ...
    async createReferralCode(c) {
        try {
            ...
            const adminCheck = await this.db.executeOne('SELECT role FROM admin_accounts WHERE user_id = ?', [authUser.id]);
            if (!adminCheck) {
            ...
            do {
                ...
                const existingCode = await this.db.executeOne('SELECT id FROM referral_codes WHERE referral_code = ? OR referral_pin = ?', [referralCode, referralPin]);
                if (!existingCode)
                    break;
                attempts++;
            } while (attempts < 10);
    ...
    async applyReferralCode(c) {
        try {
            ...
            const existingReferral = await this.db.executeOne('SELECT id FROM referrals WHERE referred_user_id = ?', [authUser.id]);
            if (existingReferral) {

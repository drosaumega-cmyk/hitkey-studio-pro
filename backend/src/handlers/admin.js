...
export class AdminHandler {
    ...
    async requireAdmin(c, requiredRole) {
        ...
        const adminCheck = await this.db.executeOne('SELECT role, permissions FROM admin_accounts WHERE user_id = ?', [authUser.id]);
        if (!adminCheck) {
    ...
    async getDashboard(c) {
        try {
            ...
            const [userStats, subscriptionStats, tokenStats, revenueStats, referralStats] = await Promise.all([
    ...
    async createSite(c) {
        try {
            ...
            const domainCheck = await this.db.executeOne('SELECT id FROM sites WHERE domain = ? OR custom_domain = ?', [domain, customDomain]);
            if (domainCheck) {

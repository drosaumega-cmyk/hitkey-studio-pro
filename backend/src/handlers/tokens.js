...
export class TokenHandler {
    ...
    async getTokenTransactions(c) {
        try {
            ...
            const result = await this.db.paginate(`SELECT * FROM token_transactions ${whereClause} ORDER BY created_at DESC`, params, parseInt(page), parseInt(limit));
    ...
    async getTokenPacks(c) {
        try {
            const packs = await this.db.execute('SELECT * FROM token_packs WHERE is_active = TRUE ORDER BY price');
            return c.json({ packs });
        }
        catch (error) {
    ...
    async purchaseTokenPack(c) {
        try {
            ...
            const tokenPack = await this.db.executeOne('SELECT * FROM token_packs WHERE id = ? AND is_active = TRUE', [tokenPackId]);
            if (!tokenPack) {
            ...
            if (!customerId) {
                const customer = await this.paymentService.createCustomer(user.email, undefined, { userId: user.id });
            ...
            const paymentIntent = await this.paymentService.createPaymentIntent(tokenPack.price, tokenPack.currency.toLowerCase(), customerId, {
                userId: user.id,
            ...
            await this.db.executeRun(`INSERT INTO user_token_packs (
    ...
    async processTokenUsage(c) {
        try {
            ...
            await this.db.executeRun(`INSERT INTO token_transactions (
          id, user_id, type, amount, token_type, description, metadata, created_at
            ...
            await this.db.executeRun(`INSERT INTO usage_analytics (
    ...
    async getSocialEngagements(c) {
        try {
            ...
            const completedEngagements = await this.db.execute(`SELECT platform, action, tokens_earned, completed_at 
         FROM social_engagements 
         WHERE user_id = ? AND verified = TRUE 
         ORDER BY completed_at DESC`, [payload.userId]);
            ...
            const availableTasksFiltered = availableTasks.filter(task => {
                return !completedEngagements.some((completed) => completed.platform === task.platform && completed.action === task.action);
            });
            return c.json({
    ...
    async submitSocialEngagement(c) {
        try {
            ...
            const existing = await this.db.executeOne(`SELECT id FROM social_engagements 
         WHERE user_id = ? AND platform = ? AND action = ? AND verified = TRUE`, [payload.userId, platform, action]);
            if (existing) {
            ...
            await this.db.executeRun(`INSERT INTO social_engagements (
          id, user_id, platform, action, tokens_earned, verification_url, 
          verified, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, FALSE, CURRENT_TIMESTAMP)`, [engagementId, payload.userId, platform, action, tokensEarned, verificationUrl]);
            // TODO: Implement verification logic
            // For now, auto-verify for demo purposes
            await this.db.executeRun(`UPDATE social_engagements 
    ...
    async calculateTokenBalance(userId) {
        const balance = await this.db.executeOne(`SELECT 
       ...
       WHERE user_id = ?`, [userId]);
        ...
        const byTypeResult = await this.db.execute(`SELECT token_type,
         SUM(CASE WHEN type IN ('earned', 'purchased', 'bonus') THEN amount ELSE 0 END) as total,
         SUM(CASE WHEN type IN ('spent', 'refund') THEN amount ELSE 0 END) as used
       FROM token_transactions 
       WHERE user_id = ?
       GROUP BY token_type`, [userId]);
        const byType = {};
    ...
    async getUsageAnalytics(c) {
        try {
            ...
            const analytics = await this.db.execute(`SELECT feature, token_type, SUM(tokens_consumed) as total_tokens, 
         ...
         ORDER BY total_tokens DESC`, params);
            const totalUsage = await this.db.executeOne(`SELECT SUM(tokens_consumed) as total_tokens, COUNT(*) as total_usage
         FROM usage_analytics 
         WHERE user_id = ? ${dateFilter}`, params);
            return c.json({
                analytics,
                summary: {

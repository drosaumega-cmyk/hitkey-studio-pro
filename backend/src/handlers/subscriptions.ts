...
export class SubscriptionHandler {
   ...
   */
  async getCurrentSubscription(c: Context) {
    try {
   ...
   */
  async createSubscriptionCheckout(c: Context) {
    try {
      const token = this.authService.extractTokenFromHeader(c.req.header('Authorization') || undefined);
      if (!token) {
        return c.json({ error: 'No token provided' }, 401);
   ...
   */
  async createPortalSession(c: Context) {
    try {
      const token = this.authService.extractTokenFromHeader(c.req.header('Authorization') || undefined);
      if (!token) {
        return c.json({ error: 'No token provided' }, 401);
   ...
   */
  async getSubscriptionHistory(c: Context) {
    try {
      const token = this.authService.extractTokenFromHeader(c.req.header('Authorization') || undefined);
      if (!token) {
        return c.json({ error: 'No token provided' }, 401);
